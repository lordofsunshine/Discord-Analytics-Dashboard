document.addEventListener("DOMContentLoaded", function() {
    ...
    confirmEdit?.addEventListener("click", editRole);
    ...
    function openDeleteModal() {
        ...
        modal.style.animation = "overlayFadeIn 0.3s ease forwards";
    ...
    async function openEditModal() {
        ...
        if (modal) {
            try {
                ...
                console.error("Ошибка при загрузке данных роли:", error);
                alert("Не удалось загрузить данные роли.");
            }
        }
    }
    ...
    function closeModal() {
        ...
        [deleteModal, editModal].forEach(modal => {
            if (modal && modal.style.display === "block") {
                modal.style.animation = "overlayFadeOut 0.3s ease forwards";
                const modalContent = modal.querySelector('.modal-content');
    ...
    async function editRole() {
        ...
        const roleColor = document.getElementById("edit-role-color").value;
        ...
        try {
            ...
            alert("Не удалось обновить роль. Пожалуйста, попробуйте снова.");
        }
    }
